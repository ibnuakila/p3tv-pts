...
$(document).ready(function () {
    ...
    $("#get-laporan").click(function (event) {
        event.preventDefault();
        var id = $(this).attr("id-laporan");
        if (confirm("Retrieve data?")) {
            $.ajax
                    ({
                        type: "POST",
                        url: url + "backoffice/kelolaregistrasi/getlaporanakhir/",
                        dataType: "json",
                        data: {id: id},
                        success: function (result)
                        {
                            if (result.response == true) {
                                alert("Retrieve berhasil");
                                location.reload();
                            } else {
...
$(document).ready(function () {
    ...
    $.ajax
            ({
                type: "GET",
                url: "https://ppptv-pts.kemdikbud.go.id/api/akreditasipt?id=" + id,
                dataType: "json",
                //data: {id:id},
                success: function (data)
                {
                    //console.log(data);
                    $("#akreditasi").val(data.result[0].nilai);
                    $("#no-sk-akre").val(data.result[0].sk_akreditasi);
                    $("#tgl-sk-akre").val(data.result[0].tgl_sk_akreditasi);
                }
            });
    $.ajax(
            {
                type: "GET",
                url: "https://ppptv-pts.kemdikbud.go.id/api/pelaporanpt?id=" + id,
                dataType: "json",
                //data: {id:id},
                success: function (data)
                {
                    ...
                    for(let i=0; i < data.result.length; i++){
                        $("#tbl-laporan tbody").append('<tr><td>'+data.result[i].id_smt+'</td>' +
                                '<td>'+data.result[i].jumlah_aktifitas+'</td>' +
                                '<td>'+data.result[i].persentase+'</td></tr>');
                    }
                }
            })
