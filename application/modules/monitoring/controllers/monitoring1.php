...
class Monitoring extends EL_Controller {
     ...
     public function edituser($id)
    {
        if ($this->session->userdata('is_loged_in') == FALSE) redirect('officer');
            ...
            $data['row'] = $this->db->query("SELECT id,nmpti,email,status,file_path FROM user_pengusul JOIN pti ON user_pengusul.kdpti=pti.kdpti where id='$id'")->row();
        ...
        if($this->input->post('ajax')==1)
        {
            ...
            if($this->form_validation->run() == FALSE)
            {
               ...
               $this->session->set_flashdata(
                  'msg', 
                  '<div class="alert alert-danger"><strong>'.$msg.'</div>'
               );               
    ...
    public function lookup() {
        $table = " SELECT id,nmpti,email,Case when status='0' then 'Non aktif' when status='1' then  'Aktif' else 'Tolak' end status,file_path,user_pengusul.kdpti FROM user_pengusul JOIN pti ON user_pengusul.kdpti=pti.kdpti ";
