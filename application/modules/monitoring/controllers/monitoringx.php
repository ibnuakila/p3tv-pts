...
class Monitoring extends EL_Controller {
    ...
    public function lookup() {
        $table = " SELECT id,kdpti,nmpti,email,CASE WHEN `status`='0' THEN 'Non aktif' WHEN `status`='1' THEN  'Aktif' ELSE 'Tolak' 
END AS `status`,file_path,kdpti  as kdpt FROM (SELECT id,nmpti,email,`status`,file_path,user_pengusul.kdpti FROM user_pengusul JOIN pti ON user_pengusul.kdpti=pti.kdpti 
