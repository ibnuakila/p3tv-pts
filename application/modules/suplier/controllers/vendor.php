 ...
 */
class Vendor extends EL_Controller {
    ...
    public function index() {
        if (!$this->sessionutility->validateSession()) {
            showBackEnd1('form_login');
        } else {

//if($this->sessionutility->validateAccess($this)){
            showBackEnd1('welcomex');
//}else{
...
//echo 'You are login';
    ...
    public function kirim($id) {
        ...
        $data['barang'] = $this->db->query("SELECT a.id_item, barang,a.spesifikasi, merk,volume - if(volume_terkirim is null,0, volume_terkirim) as volume,a.id  FROM tbl_detail_paket_hibah a JOIN tbl_item_barang b
        ON a.id_item=b.id_item WHERE id_paket='$idpaket' AND kdpti='$kdpt' and (volume - volume_terkirim) <> 0")->result();
        $data['pt'] = $this->db->query("SELECT kdpti,nmpti,alamat,kota FROM tbl_pti where kdpti='$kdpt'")->row();
        $data['title'] = "Kirim Barang ";
        $data['id'] = $id;
        showBackEnd('kirim', $data);
    }
    ...
    public function lookup() {
        ...
        $table = " SELECT id, nama_paket,no_kontrak, tgl_kontrak,tgl_akhir_kontrak,file_bukti_kontrak FROM 
            (select * from tbl_paket where id_supplier='$id' and left(tgl_kontrak,4)='2018') as dd ";
//'<a class="btn btn-primary btn-xs" href="../aliaslogin/index/'+row[0]+'" target="_blank"><span class="glyphicon glyphicon-log-in" aria-hidden="true"></span> Login</a> '+
    ...
    public function lookupkirim() {
        $id = $this->session->userdata('id_suplier');
        $table = " SELECT * FROM 
            (select id,no_resi,nmpti,tgl_kirim,tgl_terima,`status` FROM tbl_kirim a JOIN tbl_pti b ON a.kdpti=b.kdpti where id_supplier='$id') as dd ";
//'<a class="btn btn-primary btn-xs" href="../aliaslogin/index/'+row[0]+'" target="_blank"><span class="glyphicon glyphicon-log-in" aria-hidden="true"></span> Login</a> '+
        // Table's primary key
        $primaryKey = 'id';
