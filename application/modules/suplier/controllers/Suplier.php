...
//echo 'You are login';
    ...
    public function datakontrak() {
     ...
     public function datapaket() {
        ...
        if ($this->sessionutility->validateAccess($this)) {
            ...
            $data['title'] = "Status Pengiriman Paket PPPTS Periode 20201";
            showBackEndadh('list_statuspaket', $data);
        } else {
 ...
 FROM tbl_detail_paket_hibah  WHERE id_paket='$idpaket' AND kdpti='$kdpt' AND (volume - volume_terkirim) <> 0 ")->result();
    ...
    public function detailstatus($id) {
        ...
        $data['barang'] = $this->db->query("
SELECT * FROM 
            (SELECT a.type, a.spesifikasi, a.volume, jumlah,  c.no_resi,c.tgl_kirim,c.id
            FROM tbl_detail_paket_hibah a
					  left join tbl_detail_kirim b ON a.id=b.id_detail_paket_hibah
					  left join tbl_kirim c on c.id = b.id_kirim where a.id_paket='$idpaket' and a.kdpti='$kdpt') as dd")->result();
        $data['pt'] = $this->db->query("	select b.nmpti,d.nama_supplier from tbl_detail_paket_hibah a 
	join tbl_pti b on a.kdpti = b.kdpti
	join tbl_paket c on a.id_paket = c.id
	join tbl_supplier d on c.id_supplier = d.id 
	where a.kdpti='$kdpt' and a.id_paket='$idpaket'")->row();
        $data['title'] = "Status Paket ";
        
        showBackEndadh('statuspaket', $data);
    }
...
//'<a class="btn btn-primary btn-xs" href="../aliaslogin/index/'+row[0]+'" target="_blank"><span class="glyphicon glyphicon-log-in" aria-hidden="true"></span> Login</a> '+
      ...
      public function lookupstatus() {
        ...
        $table = " SELECT id_paket,nama_supplier, kdpti, nmpti,jumlah_item, jumlah_barang,presensi,periode,cek  FROM 
            (SELECT id_paket,d.nama_supplier, b.kdpti, nmpti,  COUNT(*) jumlah_item, SUM(volume) jumlah_barang,
CONCAT(SUM(IF(volume_terkirim IS NULL,0, volume_terkirim)),'/',SUM(volume)) AS presensi,e.periode,concat(b.kdpti,id_paket)as cek FROM tbl_pti a JOIN  tbl_detail_paket_hibah b ON a.kdpti=b.kdpti 
join tbl_paket c on b.id_paket = c.id
join tbl_supplier d on c.id_supplier = d.id
join registrasi e on b.id_registrasi = e.id_registrasi where e.periode='20201'

 GROUP BY c.id,b.kdpti order by id_paket) as dd ";
//'<a class="btn btn-primary btn-xs" href="../aliaslogin/index/'+row[0]+'" target="_blank"><span class="glyphicon glyphicon-log-in" aria-hidden="true"></span> Login</a> '+
        // Table's primary key
        $primaryKey = 'id_paket';
        ...
        // parameter represents the DataTables column identifier. In this case simple
        // indexes No Agenda, No TU,No surat, Asal Surat,tanggal surat, Perihal,Keterangan   , Action
//id_paket,nama_supplier, kdpti, nmpti,jumlah_item, jumlah_barang,presensi,periode,cek
        $columns = array(
            array('db' => 'id_paket', 'dt' => 0),
            array('db' => 'nama_supplier', 'dt' => 1),
            array('db' => 'kdpti', 'dt' => 2),
            array('db' => 'nmpti', 'dt' => 3),
            array('db' => 'jumlah_item', 'dt' => 4),
            array('db' => 'jumlah_barang', 'dt' => 5),
            array('db' => 'presensi', 'dt' => 6),
            array('db' => 'periode', 'dt' => 7),
            array('db' => 'cek', 'dt' => 8)
        );
    ...
    public function download($id) {
     ...
     public function download1($id) {
        $idpaket = $this->db->query("select * from tbl_kirim where id='$id'")->row();

        $name = 'bukti_kirim.jpg';
        $filepath = $idpaket->file_bukti_kirim;
